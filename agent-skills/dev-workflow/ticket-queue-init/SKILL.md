...
6. **Add `automation/logs/` to `.gitignore`** (create the entry if not already covered).
...
- **The queue must never operate on whatever's checked out interactively.** `run-queue.sh` uses a dedicated `<repo>-queue-main` sibling worktree pinned to `main`, decoupled from the user's actual working directory (which is very often a dirty feature branch mid-work). Don't collapse this back into operating directly on the user's primary checkout.
